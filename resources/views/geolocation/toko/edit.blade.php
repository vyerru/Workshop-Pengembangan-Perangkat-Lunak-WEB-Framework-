...
<div class="page-header">
    <h3 class="page-title">Edit Toko: {{ $toko->nama_toko }}</h3>
</div>
...
<div class="row">
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ route('geolocation.toko.update', $toko) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <input name="nama_toko" class="form-control @error('nama_toko') is-invalid @enderror"
                               value="{{ old('nama_toko', $toko->nama_toko) }}" required maxlength="100">
                        @error('nama_toko')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="alamat" class="form-control @error('alamat') is-invalid @enderror"
                                  rows="3" maxlength="500">{{ old('alamat', $toko->alamat) }}</textarea>
                        @error('alamat')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="map"></div>
                        <small class="text-muted">Klik pada peta atau seret marker untuk mengubah lokasi toko.</small>
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <input name="latitude" id="latitude"
                                   ...
                                   step="any" readonly
                                   value="{{ old('latitude', $toko->latitude) }}">
                            @error('latitude')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input name="longitude" id="longitude"
                                   ...
                                   step="any" readonly
                                   value="{{ old('longitude', $toko->longitude) }}">
                            @error('longitude')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4">
                            ...
                            <input name="accuracy" class="form-control @error('accuracy') is-invalid @enderror"
                                   ...
                                   value="{{ old('accuracy', $toko->accuracy) }}"
                    ...
                    <div class="mb-3">
                        <label class="form-label">Barcode Token</label>
                        <input class="form-control" value="{{ $toko->barcode_token }}" readonly>
                        <small class="text-muted">Token unik untuk generate barcode dan QR code.</small>
                    </div>
...
<script>
    ...
    function initMap() {
        const lat = parseFloat(document.getElementById('latitude').value) || -6.1754;
        const lng = parseFloat(document.getElementById('longitude').value) || 106.8272;
        const defaultLoc = { lat: lat, lng: lng };
